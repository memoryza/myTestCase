        ...
        page.open(address, function (status) {
            ...
            if (status !== 'success') {
                typeof callback == 'function' 
                    ? (page.render(filename),callback({errcode: '-1', msg: 'FAIL to load the address', page: page}))
                ...
                har = createHAR(address, page.title, page.startTime, page.resources);
                if(har.log.entries.length == 1 && Pconfig.config.errcodeList.indexOf(har.log.entries[0].response.status) >= 0) {
                    
                    typeof callback == 'function' 
                        ? (page.render(filename), callback({errcode: har.log.entries[0].response.status , msg: 'load error', page: page}))
                        : console.log('case error');
                } else {
                    typeof callback == 'function' 
                        ? callback({errcode: 0, msg: 'success', page: page, har: har})
